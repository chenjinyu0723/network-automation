...
命令模板既是用户存档，也可以在新任务中作为**可选软参考**。快照保存：拓扑图和链路接口、需求、配置思路、每台交换机最终命令，以及可选来源拓扑 ID；不会保存 thinking、内部 JSON、证据审计或 SSH 密码。模板页支持独立新建、完整编辑、增删改查、单个导入/导出和可拖动拓扑预览。

模板编辑器先选择系统已有拓扑，随后按快照内交换机动态生成命令 Tab。每个 Tab 只编辑一台交换机的命令，PC 不建 CLI 命令块；替换拓扑时保留同一设备 ID 的命令，移除消失设备的命令，新设备为空并要求用户确认。后端校验每个命令块只能绑定模板拓扑中的一个交换机且不能重复绑定。

配置规划页可选择一个参考模板。意图精炼提示词获得模板的结构摘要、需求和思路；命令计划提示词还获得模板的分设备命令摘要。当前任务的原始需求、确认思路、完整拓扑、当前设备范围和所选手册证据始终为事实优先级，提示词明确禁止照搬模板的设备名、端口、IP、网关、VLAN、厂商 CLI 或参数。模板不进入主动检索的查询词、候选页或证据选择，因此不会污染手册命令召回。
...
| 模板 | `GET/POST/PUT/DELETE /api/templates`、`POST /api/config-tasks/{id}/templates`、导入/导出接口 |
...
交接时至少验证：手册导入与重试、Embedding 构建和 FTS5 降级、拓扑保存/重开、思路为空门槛、逐设备命令生成、命令编辑审批、SSE 回显、模板独立创建/编辑/归档、参考模板软注入和 EXE 完整目录。不要把模板命令当作手册证据或自动套用到新任务，也不要让 LLM 直接获得 SQL/SSH 工具权限。
