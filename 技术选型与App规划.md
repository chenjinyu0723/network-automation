...
### 0.1 当前实现策略更新（2026-08-16）

用户流程已简化为“导入手册 → 绘制拓扑 → 在配置任务中选择已完成手册”。不再提供型号库页面，不在拓扑中选择型号，也不再用“型号 → 系列 → 手册”作为规划、审批或下发门禁。`display version` 可以由用户主动执行并会在写入前再次记录，但两次结果都仅用于审计。

本稿后文关于 S5735/S5755、型号树、系列覆盖和人工映射的内容保留为手册抽取质量研究与未来可选增强，不代表当前可执行路径。当前硬门禁是：所选手册完成抽取、写命令有该手册证据、端口来自冻结拓扑且不在保护集合、静态验证通过、用户逐台审批、设备侧验证通过后才自动 `save`。
...
3. 型号元数据：允许在 SQLite 关系表中保存“具体 SKU → 产品族 → 系列 → 手册版本 → 章节/命令”的抽取结果，作为质量分析辅助，不要求图数据库，也不作为用户门禁。
4. 混合检索：在用户选择的手册内执行 FTS5/BM25 召回 → Embedding 召回 → 规则加权/LLM 轻量重排 → 证据门禁。
...
左侧主导航：`拓扑编辑`、`手册管理`、`配置规划`、`下发与结果`、`设置`。首屏进入拓扑编辑器，不做营销落地页。顶部只显示当前项目/数据库状态、后台任务和全局错误。

典型工作路径：

```text
设置接口 → 注入手册 → 绘制拓扑 → 选择手册并创建配置任务
...
主区域是手册表格：品牌、版本、格式、文件哈希、导入时间、状态、总页/成功/失败、命令数、知识库 revision。工具栏提供导入、重试失败项、取消、归档、查看日志。
...
当前版本不提供型号库页面，也不在拓扑中选择型号。导入器保留型号候选、系列关系和适用性证据作为手册质量辅助数据；它们不参与任务创建、审批或下发门禁。

### 9.5 拓扑编辑页

使用 React Flow 全屏画布，左侧图元栏含交换机、PC；上方是保存、撤销/重做、缩放适配、删除；右侧属性面板随选择变化。

设备通用字段：名称、设备类型、IPv4/IPv6 地址、掩码/前缀、网关、备注。交换机增加：管理/SSH IP、端口、用户名和受保护端口；现场型号/版本可由用户主动调用 `display version` 填充为只读审计信息。PC 不显示 SSH 字段，`ssh_capable=false` 固定。
...
```json
{
  ...
  "revision": 3,
  "nodes": [
    {"id": "sw1", "kind": "switch", "name": "SW1", "ip": null, "prefix": null, "gateway": null, "ssh": {"host": "<SSH_IP>", "port": 22, "username": "<USERNAME>", "credential_id": "cred_x"}},
...
3. **选择手册：** 用户从已完成抽取的手册中明确选择一份作为当前任务命令上下文；未选择或手册未完成时停止。
4. **可选身份审计：** 用户主动点击时，用对应 Netmiko driver 只读执行识别命令（华为为 `display version`）并记录候选型号/版本；不让 LLM 猜 driver，也不把结果作为门禁。
5. **构建 Intent IR：** LLM 将全局需求按设备和功能域拆成原子动作；确定性代码补入拓扑端口、对端角色和地址。
6. **按需检索：** 对每个动作在所选手册内执行模式匹配、FTS5/Embedding 召回、依赖扩展和证据门禁。
7. **设备规划：** 合并同设备原子动作并拓扑排序，例如先创建 VLAN，再配置端口；生成可审计摘要。
8. **生成命令：** LLM 只能使用证据中的命令骨架与参数范围；产出正向、验证和回滚草案及逐行引用。
9. **静态验证：** 语法 AST、视图、参数范围、前置依赖、互斥、高危模式、重复/覆盖效果。
