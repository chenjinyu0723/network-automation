...
- 拓扑、手册、模板单项保存、打开、导入、导出和删除；模板可独立新建，按交换机分别维护命令，并保存可拖动拓扑、需求、思路和最终命令，不包含密码或内部模型状态。
- 配置规划可选参考模板：思路和命令阶段借鉴模板的业务拆解与命令组织，但当前拓扑、需求、用户确认思路和所选手册始终优先；模板不参与手册检索。
...
4. 在“配置规划”选择已保存拓扑和已完成手册；如有相似场景，可额外选择参考模板。输入需求并生成配置思路。审阅、修改并保存思路。
...
7. 在规划页把已完成任务保存为模板，或在“模板管理”新建模板：选择已有拓扑，填写需求和思路，再按交换机 Tab 分别填写命令。模板可查看、完整编辑、导入导出或删除。
